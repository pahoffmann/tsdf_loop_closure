...
\improvement{ggf. Mathematik dahinter erklären}. Basierend auf den berechneten Start- und Endvoxeln jedes Rays können nun die dazwischenliegenden Voxel ermittelt mittels Bresenham ermittelt werden. Abbildung \ref{pseudo:bresenham} erklärt den Pseudocode für die Bestimmung der Voxel, die die Linie gegeben durch den Startvoxel $\vec{V_{start}}$ und Endvoxel $\vec{V_{end}}$ beschrieben. Der Bresenham-Algorithmus ist

\begin{algorithm}[H]
\caption{Bresenham Algorithmus adaptiert in 3D (nach \cite{bresenham1965algorithm})} \label{pseudo:bresenham}
\begin{algorithmic}[1]
\Procedure{Bresenham}{ $\vec{V_{start}}, map_{local}$ }
	\State{\textbf{\textsl{Initialization:}}}
	\State Initialisiere die Rays $R_i$ um $P_{i}$ wie beschrieben in \ref{section:ray-tracing}
	\State Berechne die Endposition $\vec{V_{start}}^j$ für jeden Ray $r_i^j$
	...
	\State Berechne Bresenham Parameter:
	\State $dx = \abs{\vec{V_{end}}^x - \vec{V_{start}}^x}$
	\State $dy = \abs{\vec{V_{end}}^y - \vec{V_{start}}^y}$
	\State $dz = \abs{\vec{V_{end}}^z - \vec{V_{start}}^z}$
	\State $sx = \vec{V_{start}}^x < \vec{V_{end}}^x$ ? $1$ : $-1$
