...
bool RuntimeEvaluator::export_results(const boost::filesystem::path &path)
{
  ...
  while (fs::exists(finalpath))
  {
    ...
    finalpath = (parent / name).replace_extension(extension);
    tmp++;
    std::cout << "WARNING: runtime evaluator path exists. Attempting to save in " << finalpath << "\n";
  }

  return export_results(finalpath.string());
}
